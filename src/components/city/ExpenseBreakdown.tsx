    ...
    <div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="space-y-5">
          ...
          <div>
            <div className="flex justify-between text-sm mb-2"><span className="text-slate-600 font-medium flex items-center gap-2"><i className="fa-solid fa-ticket text-rose-400 w-4"></i> Activities</span><span className="font-bold text-slate-800">€{breakdown.act || (breakdown as any).activity || 0}</span></div>
            <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden"><div className="bg-rose-500 h-full rounded-full" style={{ width: `${Math.min(((breakdown.act || (breakdown as any).activity || 0) / totalBudget) * 100, 100)}%` }}></div></div>
          </div>
          <div>
