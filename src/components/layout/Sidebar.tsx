...
export default function Sidebar({ tripData, currentCityId, setCurrentCityId, onBackToTrips, onAddDestination, onOpenChecklist, onRefresh }: SidebarProps) {
  ...
  return (
    <div className={`${currentCityId ? 'hidden md:flex' : 'flex'} w-full md:w-80 bg-white border-r border-slate-200 h-full flex-col overflow-hidden shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10 relative shrink-0`}>
      <div className="p-6 border-b border-slate-100 relative">
        <button onClick={onBackToTrips} className="text-slate-400 hover:text-indigo-600 mb-4 text-sm font-medium transition-colors flex items-center gap-2">
          <i className="fa-solid fa-arrow-left"></i> Retour aux voyages
        </button>
        ...
        <div className="flex justify-between items-end mt-4">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Séjour</p>
            <p className="text-sm font-semibold">{totalNights} Nuits</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Budget Est.</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{c.nights} Nuits</p>
                  </div>
                </button>
              );
            })}
      ...
      <div className="p-4 border-t border-slate-100">
        <button onClick={onAddDestination} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-semibold py-3 rounded-xl transition-colors shadow-lg shadow-slate-900/20 flex justify-center items-center gap-2">
          <i className="fa-solid fa-plus"></i> Ajouter une destination
        </button>
        <button onClick={onOpenChecklist} className="w-full mt-3 bg-white hover:bg-slate-50 text-indigo-600 border border-indigo-200 font-semibold py-3 rounded-xl transition-colors shadow-sm flex justify-center items-center gap-2">
          <i className="fa-solid fa-list-check"></i> Checklist du voyage
        </button>
      </div>
    </div>
  );
